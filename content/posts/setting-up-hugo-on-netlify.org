...
summary: "Hugo is a static site generator that, just like other alternatives (Nicola, Jekyll, etc), allows you to write in plain text and generate HTML, JS, and CSS files. Hugo is so much simpler to use because it's a simple binary file that allows you to develop and prepare the site to be published."
tags: [ "org-mode", "hugo" ]
type: posts
aliases:
  - /blog/setting-up-hugo-on-netlify/
---

[[http://gohugo.io][Hugo]] is a static site generator that, just like other alternatives (Nicola, Jekyll, etc), allows you to write in plain text and generate HTML, JS, and CSS files.

Hugo is so much simpler to use because it's a simple binary file, called =hugo=, that allows you to develop and prepare the site to be published. Some of its features are:
...
First we'll need to install hugo. You need to install the single binary using the package manager you prefer. In macOS you can install it using Homebrew with the following command:

=brew install hugo=

You can check the version with =hugo version=, at the time I'm writing this post the latest version available is =0.48=.

Now we need to generate a new empty site so we'll use the command =hugo new site myblog=. Once the command finished you'll have a new folder called =myblog=
...
We have to change =baseURL= to =/= to avoid errors with broken links(we'll see these possible errors later). So the result =config.toml= will be:
...
- Use git submodules to create a reference to the theme repository. This way we don't need to copy all the files.
...
After that we'll see an output similar to this:
...
The server is watching for changes to be compiled so we can leave it running.
...
Once we have finished with the post it's necessary to remove =draft: true= from =hello-world.md= file otherwise the file won't show up when we publish the site.
...
Once we have the Netlify account and the site uploaded in an external repository we can proceed with the deploy.
...
By default Netlify will only build the site when we push changes to master. We can change this going to "Deploy settings" and changing "Branch deploys" options to "All" like the following image:
...
It's also possible to configure different kind of notifications(Slack, email, etc) to receive the result of the deploy.
