...
aliases:
  - /blog/code-search-alfred-workflow/
---

Code search is a useful tool to search for examples about some function, class, etc. We're going to create an Alfred workflow to easily search with [[https://github.com/features/code-search/][GitHub Code search]] and [[https://sourcegraph.com/search][SourceGraph]]

** Create a workflow

Let's go to ~Alfred settings/Workflows/+/Blank workflow~

[[file:/images/posts/code-search-alfred-workflow/create-blank-workflow.png]]

Name our new workflow, you can use whatever name you want, let's put "Code search"

[[file:/images/posts/code-search-alfred-workflow/name-new-workflow.png]]

Create a new element of type ~script filter~

[[file:/images/posts/code-search-alfred-workflow/create-script-filter-element.png]]
...
[[file:/images/posts/code-search-alfred-workflow/script-filter-complete.png]]

** Open the browser using the selected service

Now we need to define an ~action~ to open an URL in the browser

[[file:/images/posts/code-search-alfred-workflow/create-open-url-action.png]]
...
[[file:/images/posts/code-search-alfred-workflow/linked-all-together.png]]

** Result

Now if we open ~Alfred~ and write for example: ~cs completing-read~ we'll see the following image:

[[file:/images/posts/code-search-alfred-workflow/result.png]]

If we press ~enter~ a new browser tab will be opened with the selected service and input we wrote.
