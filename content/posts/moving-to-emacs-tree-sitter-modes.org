...
aliases:
  - /blog/moving-to-emacs-tree-sitter-modes/
---
...
Now using ~emacs 30~ I gave it a try to use only ~ts-modes~ and maybe delete some external packages I was using.
...
This will take all the compiled grammars by [[https://github.com/emacs-tree-sitter/tree-sitter-langs/][tree-sitter-langs]] and put them inside a directory that ~emacs~ can found them. This only works for ~macOS~, because the compiled grammars have ~.dylib~ extension but it can be adjusted to any other operating systems.

** The migration

I use ~straight.el~ to manage my packages so I'm going to use it to configure built-in ~ts-modes~, so far I've only migrated a few modes.
...
I'm in the process to migrate others modes like ~python-mode~, ~go-mode~ and so on but in some cases syntax highlighting is not the best. For example ~shell-script-mode~ has better highlight than ~bash-ts-mode~. I've been using these "new" ~ts-modes~ for a few weeks and everything is working well.
