...
aliases:
  - /blog/auto-build-and-publish-emacs-org-configuration-as-a-website/
---

Having our emacs configuration in an =org= file is great, it allow us to have it more organized and easy to read, but =org= files have more features and one of them is the ability to be exported to different formats like HTML, PDF, markdown and so on. So what if we export our emacs configuration to HTML and then publish it as a website? 🤯

It probably doesn't have any real utility but it would be nice to have an exclusive web page to show our emacs config to our friends :)
...
- Activate GitHub Pages in the extra repository, so we can have a URL where we can see the resulting website
...
This is a "regular" structure for a =dotfiles= repository, the extra =scripts= folder will be used later. Now let's dive into how it will work.

* Automate the org file rendering

We need to create two files and put them inside =scripts= folder:
...
We basically create a new org file called =index.org= and put a setup configuration file in it. You can avoid this step if you put this line directly in your config file, in this case we're using one of the themes available in this [[https://github.com/fniessen/org-html-themes][repository]], there are more themes available in this other [[https://olmon.gitlab.io/org-themes/][repository]] so you can choose the one you like the most.
...
[[file:/images/posts/auto-build-and-publish-emacs-configuration-as-a-website/set-up-github-pages.png]]
...
To be able to push changes into the new repository we have to create a personal access token, this can be made in [[https://github.com/settings/tokens/][account settings]], this token should have the =repo= scope activated.

[[file:/images/posts/auto-build-and-publish-emacs-configuration-as-a-website/generate-github-api-token.png]]
...
[[file:/images/posts/auto-build-and-publish-emacs-configuration-as-a-website/create-api-token-secret.png]]
...
[[file:/images/posts/auto-build-and-publish-emacs-configuration-as-a-website/destination-repo-commits-list.png]]

And when we enter the URL generated from GitHub Pages, [[https://erickgnavar.github.io/emacs-config/][erickgnavar.github.io/emacs-config]] in my case, we can see our configuration rendered:

[[file:/images/posts/auto-build-and-publish-emacs-configuration-as-a-website/rendered-config-result-page.png]]
