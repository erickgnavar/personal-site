...
type: posts
aliases:
  - /blog/testing-an-api-with-emacs-and-restclient/
---
...
We'll be using an example API to test the features of restclient so I prepared a little API in Flask with a few endpoints to allow us to check the features of restclient.
...
def create_item():
    if authenticated(request):
        ...
        return make_response(jsonify({"error": "please provide credentials"}), 401)
...
Now we're ready to test our API using restclient.
...
What happens if we need to pass some extra information to make an HTTP request? In restclient we can have variables and we use them in the definition of the request. In this case we'll define a =password= variable which contains the required =Authorization= value to be able to access this endpoint. Also we can define the payload of the request, in this case a json object.

First let's try a wrong password to see what happens.
...
We received a 401 response because the credentials we used are not correct. Now let's try it again but now with the correct credentials.
...
We can use =C-c C-u= from within our request to generate a =curl= command. After we execute this keybinding the =curl= command will be copied to the clipboard.
...
If we are using json as the request body we'll need to have it formatted in some way. We can use =json-mode= to accomplish this.
...
Having our requests defined in plain text allow us to use it even as documentation and we don't depend on some external app that use a custom format to store these requests. We can freely pass this =.http= file to anyone and they will be able to read it and understand it without the need to install an application.
