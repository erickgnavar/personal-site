...
tags: [ telegram, bot ]
aliases:
  - /blog/telegram-bot-to-preview-social-media-content/
---

I usually share some links of Twitter(it is known as X now but I still call it Twitter) in Telegram, but for some reason the previews doesn't work well most of the time.
...
The code is simple, it connect to Telegram API using a "personal bot", this means it will act on your behalf. It will detect when the message has a x.com link and it will replace it(making an edit) with vxtwitter.com so it will have a nice preview :)

This can be extended to do other nice things, for example: TikTok doesn't let you see videos on mobile unless you have their app installed. In case someone send me a TikTok link I automatically respond with a preview message using vm.vxtiktok.com so I can see the video inside Telegram without having to install the app :).
...
In this case we respond only to direct messages, that's why the early return when ~event.is_group~ is true.
