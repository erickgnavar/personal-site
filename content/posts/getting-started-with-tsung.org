...
aliases:
  - /blog/getting-started-with-tsung/
---

* What is tsung

Tsung is a load testing tool written in Erlang, it allows us to perform load testing for web applications, databases, etc. Or as its website says:
...
A last step is to change the Perl script a little bit, in macOS is located in =/usr/local/lib/tsung/bin/tsung_stats.pl=. We first need to change its permissions so we can edit it, we can execute =chmod 755 =/usr/local/lib/tsung/bin/tsung_stats.pl=. Now we can apply the following change:
...
Tsung load tests are defined in =xml=, in these =xml= files we can define the behaviour of the test, how many clients we want to execute, define the endpoints to be used and so on.
...
This is a simplified version of the example =http_simple.xml= example included in the installation folder, let's create a file called =demo.xml= and put this content in it:
...
This is the server who will be serving the incoming requests from =tsung=, in this case is the same machine and listening at port =4000= using a =TCP= connection, in a proper test it should be a separate machine.
...
For each execution =tsung= creates a folder under =~/.tsung/log/= where all the data generated by the tests will be saved, =tsung= also has an embedded web server where we can see the results of the tests, by default it will run on http://localhost:8091/
...
[[file:/images/posts/getting-started-with-tsung/status-page.png]]

*** Reports page

We can see stats about the connection time, response times, OS resources usage and so on.

[[file:/images/posts/getting-started-with-tsung/reports-page.png]]
...
[[file:/images/posts/getting-started-with-tsung/charts-page.png]]
...
- =csv_data=: a list of =CSV= files with all the data to be processed in some external tool like =R= or =Jupyter Notebook= for example
...
[[file:/images/posts/getting-started-with-tsung/users-arrival-chart.png]]

* Final thoughts

=tsung= can be used to test not only http services, it supports sql databases, websocket protocol and many more protocols, so we can use it to perform load tests for many parts of our application and because it is based on plain text configuration files we can version them to keep track of the tests we perform over time.
