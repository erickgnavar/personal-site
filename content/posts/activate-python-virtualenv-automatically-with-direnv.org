...
aliases:
  - /blog/activate-python-virtualenv-automatically-with-direnv/
---

[[https://direnv.net][direnv]] is a tool to set up automatically environment variables as soon as we enter a directory that contains a =.envrc= file. We can use this feature to activate our virtualenvs as well.

Let's see what happens when we activate manually a virtualenv with =source ./env/bin/activate=:

- A new environment variable called =VIRTUAL_ENV= is exported.
