...
type: posts
aliases:
  - /blog/minimal-setup-for-elixir-development-in-emacs/
---
...
To make this happen we're going to use a feature called =compile=. Compile allows us to execute a shell command and print the resulting output in a "compilation" buffer so we can see the results.
...
We get the =current-file= and =current-line= from where the function was called, and now we'll use this to build our shell command and then use this shell command to call =compile=.
...
We're going to use the latest to have a keybinding to run the test quicker.
...
- Magit: The best interface so far for using git. If you haven't used it you definitely should give it a try.
