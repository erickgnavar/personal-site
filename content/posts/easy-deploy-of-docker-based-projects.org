...
aliases:
  - /blog/easy-deploy-of-docker-based-projects/
---
...
Running all of these projects in this way it's easier to maintain and in case something happens with the server I can re deploy everything in an easy way. Let's take a look to these tools and how they work together.
...
- [[https://signup.upcloud.com/?promo=V9PZN2][Upcloud]] this will get you 25€
...
After that you need to install docker, to do that you can follow the [[https://docs.docker.com/engine/install/][official documentation]], this have specific instructions for your Linux distribution.
...
We need to log in with the =postgres= user so we can enter a =psql= session. We can do it with:
...
Now when we reload our caddy server with =sudo systemctl reload caddy= it will get the SSL certificates and internally will check if they are still valid, otherwise it will renew them.

* Running our projects with docker-compose

[[https://docs.docker.com/compose/][Docker compose]] is a tool that allow us to define different docker services in an easier way using a =yaml= file.
...
This can be done on a separate machine because once the images are pushed to a remote registry they can be downloaded in our server.
