...
tags: [ k8s, shell ]
aliases:
  - /blog/creating-helper-commands-for-k8s-using-fzf/
---
...
I used the power of [[https://github.com/junegunn/fzf][fzf]] to create an interactive experience when I run any of these functions. Basically it pipes the output of a =kubectl= command, do some filtering using =sed= and =awk= and then build a final command which will execute what I want.

The common behavior of these functions is first ask for a =namespace= and then ask for a specific =pod= to make some action on it.
...
These are some common tasks I need in a day of work but with the same logic we can build some other commands.
