...
type: posts
aliases:
  - /blog/save-and-restore-window-configuration-in-emacs/
---

When we are using =emacs= sometimes we often open many buffers in different windows at the same time, for example we can have a buffer for an opened file, another for seeing test results and so on.
