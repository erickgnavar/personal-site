...
To solve this problem we can use a =register= to save the current window configuration just before the source code edition buffer is opened and when it's closed we can restore the previous configuration just getting the value from the =register= used before. We're defining two functions to accomplish this behavior, one to run before and one after we're done editing the source block.
